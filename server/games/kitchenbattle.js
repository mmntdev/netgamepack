...
// 3分間でチーム合計スコアが高いほうの勝ち。提供窓口と中央カウンターは両チーム共用。
...
// '.'=床 '#'=作業台 L/T/O=食材箱 C=まな板 P=コンロ D=皿 W=提供窓口(共用) X=ゴミ箱
...
class KitchenBattleGame {
  ...
  action(p) {
    ...
    if (ch === 'D') {
      ...
      else if (carry.type === 'plate' && carry.contents.length === 0) p.carry = null;
      return;
    }

    // 提供窓口(共用)— 先に出したチームが得点を奪う
    if (ch === 'W') {
...
module.exports = {
  meta: {
    ...
    name: 'キッチンバトル 2vs2',
    description:
      'チーム対抗の料理バトル!左右対称のキッチンに分かれ、共有の注文を早い者勝ちで取り合う。先に提供したチームが得点。3分間で合計スコアが高いチームの勝ち。中央のカウンターは共用なので、置いた食材は取られるかも!?(2〜4人・途中参加OK)',
    minPlayers: 2,
    maxPlayers: 4,
